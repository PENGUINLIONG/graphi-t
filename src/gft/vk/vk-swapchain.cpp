...
fmt::Format _get_valid_format(
  ...
  L_ASSERT(
    ...
    "surface format is not supported by the underlying platform"
  );

  return selected_fmt2;
}

uint32_t _get_valid_image_count(
  const VulkanContext& ctxt,
  const SwapchainConfig& cfg
) {
  VkSurfaceCapabilitiesKHR sc{};
  VK_ASSERT << vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
    ctxt.physdev(), ctxt.surf->surf, &sc
  );
...
std::unique_ptr<SwapchainDynamicDetail> _create_swapchain_dyn_detail(
...
void _collect_swapchain_images(
  const VulkanSwapchain& swapchain,
  ...
  for (uint32_t i = 0; i < image_count; ++i) {
    ...
    img_info.usage = L_IMAGE_USAGE_ATTACHMENT_BIT | L_IMAGE_USAGE_PRESENT_BIT;

    VulkanImageRef out =
      std::make_shared<VulkanImage>(swapchain.ctxt, std::move(img_info));
    out->img = std::make_shared<sys::Image>(
      *swapchain.ctxt->allocator, img, nullptr, false
    );
...
void _acquire_swapchain_img(VulkanSwapchain& swapchain) {
  ...
  VkResult acq_res = vkAcquireNextImageKHR(
    ctxt.dev->dev,
    *swapchain.swapchain,
    SPIN_INTERVAL,
    VK_NULL_HANDLE,
  ...
  VK_ASSERT << vkWaitForFences(
    ctxt.dev->dev, 1, &fence, VK_TRUE, SPIN_INTERVAL
  );
...
SwapchainRef VulkanSwapchain::create(
  ...
  VulkanSwapchainRef out =
    std::make_shared<VulkanSwapchain>(ctxt_, std::move(info));
...
ImageRef VulkanSwapchain::get_current_image() {
  L_ASSERT(
    dyn_detail != nullptr,
    "swapchain recreation is required; call `acquire_swapchain_img` "
    "first"
  );

  L_ASSERT(
    *dyn_detail->img_idx != ~0u,
...
uint32_t VulkanSwapchain::get_width() const {
  L_ASSERT(
    dyn_detail != nullptr,
    "swapchain recreation is required; call `acquire_swapchain_img` "
    "first"
  );
  return dyn_detail->width;
}
uint32_t VulkanSwapchain::get_height() const {
  L_ASSERT(
    dyn_detail != nullptr,
    "swapchain recreation is required; call `acquire_swapchain_img` "
    "first"
  );
  return dyn_detail->height;
}
...
InvocationRef VulkanSwapchain::create_present_invocation(
