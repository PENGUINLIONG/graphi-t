  ...
  for (uint32_t i = 0; i < attm_cfgs.size(); ++i) {
    ...
    switch (attm_cfg.attm_ty) {
      ...
      case L_ATTACHMENT_TYPE_DEPTH: {
        L_ASSERT(
          !sar.has_depth_attm, "subpass can only have one depth attachment"
        );
  ...
  for (size_t i = 0; i < attms.size(); ++i) {
    ...
    switch (attm.rsc_view_ty) {
      ...
      case L_RESOURCE_VIEW_TYPE_DEPTH_IMAGE:
        img_views.at(i) =
          *VulkanDepthImage::from_hal(attm.depth_img_view.depth_img)->img_view;
        break;
      default:
...
RenderPassRef VulkanRenderPass::create(
  ...
  VulkanRenderPassRef out =
    std::make_shared<VulkanRenderPass>(ctxt_, std::move(info));
  out->ctxt = ctxt_;
    ...
    switch (rsc_view.rsc_view_ty) {
      case L_RESOURCE_VIEW_TYPE_IMAGE: {
        VkImageView img_view =
          VulkanImage::from_hal(rsc_view.img_view.img)->img_view->img_view;
        ss << "," << img_view;
        break;
      }
      case L_RESOURCE_VIEW_TYPE_DEPTH_IMAGE: {
        VkImageView img_view =
          VulkanDepthImage::from_hal(rsc_view.depth_img_view.depth_img)
            ->img_view->img_view;
        ss << "," << img_view;
        break;
      }
      default:
        L_PANIC("unsupported resource type as an attachment");
    }
  }
  return {ss.str()};
}
...
TaskRef VulkanRenderPass::create_graphics_task(const GraphicsTaskConfig& cfg) {
  return VulkanTask::create(shared_from_this(), cfg);
}

InvocationRef VulkanRenderPass::create_render_pass_invocation(
