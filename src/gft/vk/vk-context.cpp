...
sys::SurfaceRef _create_surf_windows(
  const VulkanInstance& inst,
  const ContextWindowsConfig& cfg
) {
#if VK_KHR_win32_surface
  L_ASSERT(
    cfg.dev_idx < inst.physdev_details.size(),
    "wanted vulkan device does not exists (#",
    cfg.dev_idx,
    " of ",
    inst.physdev_details.size(),
    " available devices)"
  );
...
sys::SurfaceRef _create_surf_android(
  const VulkanInstance& inst,
...
sys::SurfaceRef _create_surf_metal(
  const VulkanInstance& inst,
  const ContextMetalConfig& cfg
) {
#if VK_EXT_metal_surface
  L_ASSERT(
    cfg.device_index < inst.physdev_details.size(),
    "wanted vulkan device does not exists (#",
    cfg.device_index,
    " of ",
    inst.physdev_details.size(),
...
ContextRef _create_ctxt(
  ...
  L_ASSERT(
    dev_idx < inst->physdev_details.size(),
    "wanted vulkan device does not exists (#",
    dev_idx,
    " of ",
    inst->physdev_details.size(),
  ...
  for (uint32_t i = 0; i < qfam_props.size(); ++i) {
    ...
    L_DEBUG(
      "discovered queue families #", i, ": ", util::join(" | ", qfam_cap_lit)
    );

    uint32_t nset_bit = util::count_set_bits(queue_flags);
    auto it = qfam_map.find(nset_bit);
    if (it == qfam_map.end()) {
      it = qfam_map.emplace_hint(
        it, std::make_pair(nset_bit, std::vector<QueueFamilyTrait>{})
      );
    }
    it->second.emplace_back(QueueFamilyTrait{i, queue_flags});
  }
  L_ASSERT(
    !qfam_props.empty(), "cannot find any queue family on device #", dev_idx
  );
  ...
  std::vector<SubmitTypeQueueRequirement> submit_ty_reqs{
    SubmitTypeQueueRequirement{
      L_SUBMIT_TYPE_ANY,
      "ANY",
      [](const QueueFamilyTrait& qfam_trait) { return true; },
    },
    SubmitTypeQueueRequirement{
    ...
    SubmitTypeQueueRequirement{
      ...
      [&](const QueueFamilyTrait& qfam_trait) {
        ...
        VK_ASSERT << vkGetPhysicalDeviceSurfaceSupportKHR(
          physdev, qfam_trait.qfam_idx, surf->surf, &is_supported
        );
  ...
  for (size_t i = 0; i < submit_ty_reqs.size(); ++i) {
    ...
    if (qfam_idx_alloc == VK_QUEUE_FAMILY_IGNORED) {
      L_WARN(
        "cannot find a suitable queue family for ",
  ...
  for (const auto& pair : queue_allocs) {
    ...
    if (qfam_idx == VK_QUEUE_FAMILY_IGNORED) {
      continue;
    }
  ...
  img_samplers[L_IMAGE_SAMPLER_LINEAR] = sys::create_sampler(
    dev->dev,
    VK_FILTER_LINEAR,
  ...
  img_samplers[L_IMAGE_SAMPLER_NEAREST] = sys::create_sampler(
    dev->dev,
    VK_FILTER_NEAREST,
  ...
  img_samplers[L_IMAGE_SAMPLER_ANISOTROPY_4] = sys::create_sampler(
    dev->dev,
    VK_FILTER_LINEAR,
    VK_SAMPLER_MIPMAP_MODE_LINEAR,
    4.0f,
  ...
  depth_img_samplers[L_DEPTH_IMAGE_SAMPLER_LINEAR] = sys::create_sampler(
    dev->dev,
    VK_FILTER_LINEAR,
    VK_SAMPLER_MIPMAP_MODE_LINEAR,
    0.0f,
  ...
  depth_img_samplers[L_DEPTH_IMAGE_SAMPLER_NEAREST] = sys::create_sampler(
    dev->dev,
    VK_FILTER_NEAREST,
    VK_SAMPLER_MIPMAP_MODE_NEAREST,
    0.0f,
  ...
  depth_img_samplers[L_DEPTH_IMAGE_SAMPLER_ANISOTROPY_4] = sys::create_sampler(
    dev->dev,
    VK_FILTER_LINEAR,
    VK_SAMPLER_MIPMAP_MODE_LINEAR,
    4.0f,
...
ContextRef VulkanContext::create(
...
DescriptorSetPoolItem VulkanContext::acquire_desc_set(
  const std::vector<ResourceType>& rsc_tys
) {
  ...
  if (desc_set_detail.desc_set_pool.has_free_item(key)) {
    ...
    sys::DescriptorSetRef desc_set = _alloc_desc_set(
      *this, desc_pool->desc_pool, desc_set_layout->desc_set_layout
    );
    desc_set_detail.desc_pools.emplace_back(std::move(desc_pool));
    return desc_set_detail.desc_set_pool.create(
      std::move(key), std::move(desc_set)
    );
  }
}

sys::CommandPoolRef _create_cmd_pool(
  const VulkanContext& ctxt,
  SubmitType submit_ty
) {
...
QueryPoolPoolItem VulkanContext::acquire_query_pool() {
  if (query_pool_pool.has_free_item(0)) {
    ...
    sys::QueryPoolRef query_pool =
      _create_query_pool(*this, VK_QUERY_TYPE_TIMESTAMP, 2);
...
SwapchainRef VulkanContext::create_swapchain(const SwapchainConfig& cfg) {
  return VulkanSwapchain::create(shared_from_this(), cfg);
}
