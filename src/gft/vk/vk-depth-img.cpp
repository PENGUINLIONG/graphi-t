...
DepthImageRef VulkanDepthImage::create(
  ...
  ivci.subresourceRange.aspectMask =
    (fmt::get_fmt_depth_nbit(depth_img_cfg.depth_format) > 0
       ? VK_IMAGE_ASPECT_DEPTH_BIT
       : 0) |
    (fmt::get_fmt_stencil_nbit(depth_img_cfg.depth_format) > 0
  ...
  VulkanDepthImageRef out =
    std::make_shared<VulkanDepthImage>(ctxt_, std::move(info));
